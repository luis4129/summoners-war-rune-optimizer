...
class PrefixStatBonusServiceTest {
    ...
    private static final BigDecimal SPEED_BONUS = BigDecimal.valueOf(6);
    private static final BigDecimal ATTACK_BONUS = BigDecimal.valueOf(48).setScale(2, RoundingMode.UP);

    @Test
    void shouldCalculatePrefixStatBonusWithSumAggregator() {
        //given
        var build = BuildFactory.getBuildWithSpeedAttribute();

        //when
        var prefixStatBonus = prefixStatBonusService.getPrefixStatBonus(SPEED_ATTRIBUTE, BASE_SPEED_VALUE, build);

        //then
        assertEquals(SPEED_BONUS, prefixStatBonus);
    }

    @Test
    void shouldCalculatePrefixStatBonusWithMultiplyAggregator() {
        //given
        var build = BuildFactory.getBuildWithAttackAttribute();

        //when
        var prefixStatBonus = prefixStatBonusService.getPrefixStatBonus(ATTACK_ATTRIBUTE, BASE_ATTACK_VALUE, build);

        //then
        assertEquals(ATTACK_BONUS, prefixStatBonus);
    }
}