...
class MainStatBonusServiceTest {
    ...
    private static final BigDecimal SPEED_BONUS = BigDecimal.valueOf(42);
    private static final BigDecimal ATTACK_BONUS = BigDecimal.valueOf(378).setScale(2, RoundingMode.UP);

    @Test
    void shouldCalculateMainStatBonusWithSumAggregator() {
        //given
        var build = BuildFactory.getBuildWithSpeedAttribute();

        //when
        var mainStatBonus = mainStatBonusService.getMainStatBonus(SPEED_ATTRIBUTE, BASE_SPEED_VALUE, build);

        //then
        assertEquals(SPEED_BONUS, mainStatBonus);
    }

    @Test
    void shouldCalculateMainStatBonusWithMultiplyAggregator() {
        //given
        var build = BuildFactory.getBuildWithAttackAttribute();

        //when
        var mainStatBonus = mainStatBonusService.getMainStatBonus(ATTACK_ATTRIBUTE, BASE_ATTACK_VALUE, build);

        //then
        assertEquals(ATTACK_BONUS, mainStatBonus);
    }
}